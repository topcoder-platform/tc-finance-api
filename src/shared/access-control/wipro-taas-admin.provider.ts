...
@Injectable()
export class WiproTaasAdminProvider implements RoleAccessProvider {
  ...
  async applyFilter<T>(_userId: string, req: any): Promise<T> {
    return { ...req, category: winnings_category.TAAS_PAYMENT } as T;
  }
  ...
  async verifyAccessToResource(winningsId: string | string[], _userId: string) {
    ...
    const unauthorized = winnings.filter(
      (w) => w.category !== winnings_category.TAAS_PAYMENT,
    );
    if (unauthorized.length > 0) {
      throw new Error(
        `${Role.WiproTaasAdmin} user is trying to access winning with category='${unauthorized.map((w) => w.category).join(', ')}'`,
      );
    }
  }
}
